...
const client::JsonCppArray<Binding > Policy::get_bindings() const {
   const Json::Value& storage = Storage("bindings");
   return client::JsonValueToCppValueHelper<client::JsonCppArray<Binding > >(storage);
}

client::JsonCppArray<Binding > Policy::mutable_bindings() {
  Json::Value* storage = MutableStorage("bindings");
  return client::JsonValueToMutableCppValueHelper<client::JsonCppArray<Binding > >(storage);
}
}  // namespace google_pubsub_api
